...
export function buildTailoredCvSystemPrompt(
  ...
  if (relevantExperiences && relevantExperiences.length > 0) {
    const entries = relevantExperiences
      ...
      .join('\n')
    parts.push(
      `Here are the user's written STAR stories for key interview themes. Reference these concrete examples when suggesting how to strengthen the CV:\n<relevant_experiences>\n${entries}\n</relevant_experiences>`,
    )
  }
  ...
  if (hasDraft) {
    parts.push(`Here is the user's current CV draft:\n<current_draft>\n${editorContent}\n</current_draft>`)
    parts.push(
      `The user has an existing draft. When they ask for revisions or a rewrite, output the complete updated CV using the format below. For general feedback or questions, respond as plain text without the tags.\n\n${EDITOR_CONTENT_FORMAT}`,
    )
  } else {
    parts.push(
      `The user has not written anything yet. When they ask you to tailor their CV, generate a tailored draft using the format below. For questions or guidance, respond as plain text without the tags.\n\n${EDITOR_CONTENT_FORMAT}`,
    )
  }
